...
nlohmann::json LogAnalysisService::calculateStatistics(const std::vector<models::LogEntry> &entries)
{
    ...
    int errorCount = levelCounts.value("ERROR", 0) + levelCounts.value("CRITICAL", 0) + levelCounts.value("FATAL", 0);
...
bool LogAnalysisService::saveAnalysisResult(int logFileId, int userId, const AnalysisResult &result)
{
    try
    {
        dbClient_->execSqlSync(
            ...
            result.detectedPatterns, result.isAnomaly);
...
AnalysisResult LogAnalysisService::analyzeUnusualPatterns(const std::vector<models::LogEntry> &entries)
{
    ...
    static const std::vector<std::pair<std::regex, std::string>> criticalPatterns = {
        {std::regex(R"(out\s+of\s+memory)", "OutOfMemory"},
        {std::regex(R"(disk\s+full)", "DiskFull"},
        {std::regex(R"(connection\s+(refused|timeout|lost))", "ConnectionIssue"},
        {std::regex(R"(authentication\s+failed)", "AuthFailed"},
        {std::regex(R"(permission\s+denied)", "PermissionDenied"},
        {std::regex(R"(segmentation\s+fault)", "SegFault"},
    };

    std::map<std::string, int> patternMatches;
    for (const auto &entry : entries)
    {
        for (const auto &[pattern, name] : criticalPatterns)
        {
            if (std::regex_search(entry.message, pattern, std::regex_constants::icase))
            {
                patternMatches[name]++;
            }
        }
