                                       ...
                                       std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    try
    {
        ...
        for (const auto &row : result)
        {
            ...
            item["detected_patterns"] = row["detected_patterns"].as<std::vector<std::string>>();
                                     ...
                                     std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    try
    {
        ...
        std::string pattern = (*json)["pattern"].get<std::string>();
        std::string severity = (*json).value("severity", "medium").get<std::string>();
